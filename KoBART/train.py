...
class Base(pl.LightningModule):
    ...
    def configure_optimizers(self):
        ...
        optimizer = AdamW(optimizer_grouped_parameters,                      #옵티마이저 : 손실함수기반으로 네트워크가 어떻게 업데이트 될지 결정 :확률적 경사 하강법(SGD)
...
class KoBARTConditionalGeneration(Base):
    ...
    def forward(self, inputs): # forward는 모델의 추론 결과를 제공하고 싶을 때 사용
        ...
        return self.model(input_ids=inputs['input_ids'],
                          ...
                          labels=inputs['labels'].long(), return_dict=True)        # long()으로 수정    


    def training_step(self, batch, batch_idx):
        #print(dir(batch))  #  dict_keys(['loss', 'logits', 'past_key_values', 'encoder_last_hidden_state'])
        outs = self(batch)
        outs.logits
        #print(outs.logits)
    ...
    def validation_step(self, batch, batch_idx):
        ...
        outs = self(batch)
        #print(outs.keys())
        #print(outs['logits'])   #  dict_keys(['loss', 'logits', 'past_key_values', 'encoder_last_hidden_state'])
        loss = outs['loss']
        #loss.requires_grad_(True)
        return (loss)
